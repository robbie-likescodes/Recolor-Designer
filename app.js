/* Palette Mapper — Cup Print Helper (FULL JS v9 + Final Inks PMS Report)
   Includes:
   ...
   >>> NEW:
   - Final Inks (post-substitution) PMS report for printer handoff.
*/
...
const els = {
  ...
  // (Optional) Replacement rules table body if you render one:
...
const state = {
  ...
  // If you maintain live replacement rules in code (not only DOM), keep them here:
  // textureRules: [{ targetHex, mode, inks:[hex...], weights:[...], enabled:true }, ...]
  textureRules: [], // leave empty if you only render rules in the DOM
};
...
function renderHalftone(ctx, imgData, palette, bgHex, cellSize=6, jitter=false, wL=1.0, wC=1.0, effRegions=[]){
  ...
  const palLab=buildPaletteLab(palette);
  const bg=hexToRgb(bgHex)||{r:255,g:255,b:255}; const bgLab=rgbToLab(bg.r,b.g?bg.g:0,b.b?b.b:0); // minor guard
...
// (Original) Report helpers (kept in case you want the original list somewhere)
...
/* >>> FINAL INKS (PMS) ADDITIONS — START <<< */
/** Get UI/State replacement rules (generic) */
function collectActiveReplacements(){
  if (Array.isArray(state.textureRules) && state.textureRules.length){
    return state.textureRules.filter(r => r && r.enabled);
  }
  const tbody = els.rulesTbody || document.getElementById('rulesTbody');
  if (!tbody) return [];
  const rules = [];
  tbody.querySelectorAll('tr').forEach(tr=>{
    const on = tr.querySelector('input[type=checkbox][data-role="enable"]');
    if (on && !on.checked) return;
    const target = (tr.getAttribute('data-target-hex') || '').toUpperCase();
    const inks = (tr.getAttribute('data-inks') || '')
      .split(',')
      .map(s=>s.trim().toUpperCase())
      .filter(h=>/^#([0-9A-F]{6})$/.test(h));
    if (target && inks.length) {
      rules.push({ targetHex: target, inks, enabled: true });
    }
  });
  return rules;
}
/** Compute final ink set after substitutions */
function finalInkPaletteHexes(){
  const orig = getPalette().map(([r,g,b]) => rgbToHex(r,g,b).toUpperCase());
  const final = new Set(orig);
  const rules = collectActiveReplacements();
  for (const r of rules){
    final.delete(r.targetHex.toUpperCase());
    r.inks.forEach(h => final.add(h.toUpperCase()));
  }
  return Array.from(final);
}
/** PMS mapping for final inks */
function currentFinalPaletteCodes(){
  return finalInkPaletteHexes().map(hex=>{
    const p = nearestPms(hex);
    return { hex, pms: p.name, pmsHex: p.hex, deltaE: p.deltaE };
  });
}
/** Final report for printer */
function buildPrinterReportFinal(){
  const items = currentFinalPaletteCodes();
  const lines = [
    'Project: Palette Mapper output',
    `Final inks used (post-substitution): ${items.length}`,
    '—',
    ...items.map((c,i)=> `${i+1}. ${c.pms} (${c.pmsHex}) • from ${c.hex} • ΔE≈${c.deltaE.toFixed(1)}`)
  ];
  return lines.join('\n');
}
/* >>> FINAL INKS (PMS) ADDITIONS — END <<< */
...
function wireCodesUI(){
  ...
  els.exportReport?.addEventListener('click', ()=>{
    // >>> Use FINAL inks report for printer handoff
    const txt = buildPrinterReportFinal();
    const blob = new Blob([txt], {type:'text/plain'});
    const a = document.createElement('a');
    a.download = 'final_palette_pms_report.txt';
...
/* >>> FINAL inks in the on-screen list (optional; remove if you want original palette there) */
function renderCodeList(){
  const box = els.codeList;
  if (!box) return;
  const rows = currentFinalPaletteCodes().map((c,i)=>(
    `<div class="row">
      <span class="sw" style="display:inline-block;width:14px;height:14px;border-radius:3px;border:1px solid #334155;background:${c.pmsHex}"></span>
      ${i+1}. ${c.pms} (${c.pmsHex}) • from ${c.hex} • ΔE≈${c.deltaE.toFixed(1)}
    </div>`
  ));
  box.innerHTML = rows.join('') || '<em>No inks</em>';
}

/* Email to printer now references FINAL inks */
function updateMailto(){
  const to = '';
  const subject = encodeURIComponent('Print job: artwork + FINAL PMS palette');
  const preview = buildPrinterReportFinal().split('\n').slice(0, 24).join('\n');
  const body = encodeURIComponent(
`Hi,

Please find attached the artwork PNG (full resolution) and the FINAL PMS palette list (after substitutions/mixes).
...
function bindEvents(){
  ...
  els.applyBtn?.addEventListener('click', async ()=>{
    ...
    // Refresh report/mail links (e.g., if rules affect final inks)
    renderCodeList();
    updateMailto();
  });
